...
std::shared_ptr<AnisetteData> AnisetteDataManager::FetchAnisetteData()
{
	...
	while (retryCount < MAX_RETRIES) {
		try {
			// Tạo client MỚI mỗi lần thử (fix 503)
			...
			auto task = client.request(request)
				.then([=](http_response response)
					{
						// Kiểm tra 503 - throw để retry
						if (response.status_code() == 503 || response.status_code() >= 500) {
							odslog("Got " << response.status_code() << " - retrying...");
							throw std::runtime_error("Got 503/5xx");
						}
			...
			// Đóng client để giải phóng kết nối
			client.close();
			
			// Nếu thành công, trả về
			if (anisetteData != NULL) {
				odslog("Successfully fetched anisette data");
				return anisetteData;
			}
		} catch (const std::exception& e) {
			odslog("Error: " << e.what());
			retryCount++;
			odslog("Retry " << retryCount << "/" << MAX_RETRIES);
			std::this_thread::sleep_for(std::chrono::seconds(retryCount * 2));
		}
	}
	
	odslog("Failed to fetch anisette data after " << MAX_RETRIES << " retries");
	return NULL;
}
